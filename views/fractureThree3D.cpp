...
void FractureThree3D::initialize() {
  ...
  for(int i=7;i<11;i++) {
    for(int j=7;j<11;j++) {
      periSystem->getPointMasses()->get(i*16+j)->getExternelForces()->add(new Vec3d(0.0,0.0,-10.0));
    }
  }
}

void FractureThree3D::deInitialize() {
  delete periSystem;
}

void FractureThree3D::keyboard(int key,int scancode,int action,int mods) {
...
void FractureThree3D::mouseClick(int button,int action,int mods) {
  // to be implemented
}

void FractureThree3D::display() {
  ...
  for(int i=7;i<11;i++) {
    for(int j=7;j<11;j++) {
      periSystem->getPointMasses()->get(i*16+j)->render(WIREFRAME);
    }
  }
...
void FractureThree3D::doPhysicsStep(double dt) {
